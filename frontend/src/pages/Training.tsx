...
import { Button, Card, Col, Descriptions, Divider, Empty, Form, Input, InputNumber, Progress, Row, Select, Space, Statistic, Table, Tag, Typography, message } from 'antd'
...
export default function Training() {
  ...
  const refreshModels = async () => {
    try {
      ...
      message.error(`加载模型列表失败：${e instanceof Error ? e.message : String(e)}`)
    }
  }
  ...
  const startPolling = (jobId: string) => {
    const tick = async () => {
      try {
        ...
        message.error(`获取训练进度失败：${e instanceof Error ? e.message : String(e)}`)
      }
    }
    void tick()
  }
  ...
  const onCreateJob = async () => {
    try {
      ...
      if (v.fewshotSize > FEWSHOT_MAX) {
        message.warning(`小样本训练集大小建议不超过 ${FEWSHOT_MAX}，已自动截断`)
  ...
  const totalEpochs = parsedParams.epochs ?? (metrics.length ? metrics[metrics.length - 1]!.epoch : 0)
  const currentEpoch = latestMetric?.epoch ?? 0
  const progressPercent =
    totalEpochs && currentEpoch ? Math.min(100, Math.round((currentEpoch / totalEpochs) * 100)) : 0
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card bordered={false} style={{ borderRadius: 12 }} styles={{ body: { padding: 20 } }}>
        ...
        <Row gutter={[16, 12]} align="middle">
          <Col flex="auto">
            ...
            <Typography.Text type="secondary">
              在样本量≤64的条件下，基于选定的提示模板进行提示调优训练，并实时查看 loss/acc/F1
              变化。
            </Typography.Text>
          </Col>
        </Row>
        ...
        <Row justify="space-between" align="middle" style={{ marginBottom: 12 }}>
          ...
          <Col>
            <Button type="primary" loading={creating} onClick={() => void onCreateJob()}>
              启动训练
            </Button>
          </Col>
        </Row>
        ...
        >
          <Row gutter={16}>
            ...
            <Col xs={24} lg={4}>
              <Form.Item
                label={`小样本训练集大小（≤${FEWSHOT_MAX}）`}
                name="fewshotSize"
                rules={[{ required: true, message: '请输入样本数量' }]}
              >
                <InputNumber min={1} max={FEWSHOT_MAX} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col xs={24} lg={4}>
              <Form.Item
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={16}>
            ...
            {metrics.length ? (
              <Line
                ...
                yAxis={{ title: { text: '指标值' }, min: 0, max: 1, tickCount: 6 }}
                legend={{ position: 'top' }}
          ...
          <Col xs={24} lg={8}>
            ...
            {currentJob ? (
              <>
                ...
                >
                  ...
                  <Descriptions.Item label="状态">
                    <Tag color={statusColor[currentJob.status]}>{statusText(currentJob.status)}</Tag>
                  </Descriptions.Item>
              ...
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无训练任务" />
            )}
          </Col>
        </Row>
